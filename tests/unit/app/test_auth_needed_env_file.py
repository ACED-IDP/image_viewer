...
from tests.unit.app.conftest import monkey_patch_mocks
...
def test_base_url_from_env_file(monkeypatch, client_with_cookie_env_file):
    ...
    assert response.headers["location"].startswith(expected_base_url), response.headers["location"]
    assert response.headers["location"].endswith(object_id), response.headers["location"]
